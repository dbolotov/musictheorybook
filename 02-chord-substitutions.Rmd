...
Using alterations (described in the **Altered Scale** section) is a standard way to add tension, and one way of remembering these alterations is to simply memorize voicings that contain them. Another way is to use chord substitutions - similarly to supersimpositions, we can use familiar shapes to generate new voicings.
...
# - **Note on Resolutions to Major and Minor:** While it is not illegal to use alterations on a dominant resolving to a minor i chord, tension in such dominants is usually created by adding extensions from the Harmonic Minor Scale. For example, in the progression D7-Gm, superimposing F#dim7 from Gm Harmonic Minor over D7 creates a D7b9 sound. This is because in a minor V7-i in jazz, the V7 harmony usually comes from Harmonic Minor (and not Natural Minor, where the V would not be a dominant chord). To summarize: in major V7-I, alterations on the dominant come from outside the parent Major Scale, while in minor V7-i, they come from the parent Harmonic Minor Scale (and are extensions). 
...
### Dominants b3 Away {-}
A functioning dominant can be substituted by any other dominant that is located a multiple of a minor 3rd away. Barry Harris refers to this as "playing with your sisters and brothers". This is a different wa of thinking about alterations than using the Altered Scale.
